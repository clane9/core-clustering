...
def main():
  ...
  printformstr = ('(epoch {:d}/{:d}) lr={:.3e} err={:.4f} obj={:.3e} '
      'loss={:.3e} reg={:.3e} samp/s={:.0f}')
  logheader = 'Epoch,LR,Err,Obj,Loss,Reg,Samp.s'
  logformstr = '{:d},{:.9e},{:.9f},{:.9e},{:.9e},{:.9e},{:.0f}'
...
def train_epoch(model, objfun, data_loader, device, optimizer):
  model.train()
  obj, loss, reg, sampsec = [ut.AverageMeter() for _ in range(4)]
  tic = time.time()
  for kk, (ii, x) in enumerate(data_loader):
    # forward
    ii, x = ii.to(device), x.to(device)
    batch_obj, batch_loss, batch_reg = objfun(model, ii, x)
    batch_size = x.size(0)
  ...
  return obj.avg, loss.avg, reg.avg, sampsec.avg
